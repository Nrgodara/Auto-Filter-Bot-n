    ...
    if settings['shortlink']:
        btn.insert(0,
            ...
            #InlineKeyboardButton("🥇 ʙᴜʏ 🥇", url=f"https://t.me/{temp.U_NAME}?start=plans")
             InlineKeyboardButton("🥇 ʙᴜʏ 🥇", callback_data="buy_premium"),
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if query.from_user.id not in ADMINS:
            return await query.answer("You are not my admin. These Commands work for ADMINS Only!", show_alert=True)
        buttons = [[
            InlineKeyboardButton('« ʙᴀᴄᴋ', callback_data='help')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
